...
{
    public class DevicesParser : SectionParser
    {
        public DevicesParser(CommandLineLogger logger, ExtractFileInfo fileInfo, bool isPreservation, string preservationPrefix, ParserVersionEnum version)
        ...
        private IEnumerable<Device> Items { get; set; }
        #endregion
        ...
        public override IEnumerable<DataTable> GenerateDataTables()
        {
            ...
            data.Columns.Add("DeviceType", typeof(string));
            data.Columns.Add("DeviceId", typeof(string));
            ...
            foreach (Device item in Items.Where(x => x.HasData))
            {
                DataRow row = data.NewRow();

                row["DeviceType"] = !string.IsNullOrEmpty(item.DeviceType) ? item.DeviceType : null;
                row["DeviceId"] = !string.IsNullOrEmpty(item.DeviceId) ? item.DeviceId : null;
                row["IsActive"] = !string.IsNullOrEmpty(item.IsActive) ? item.IsActive : null;
                row["File"] = SourceFile;
                data.Rows.Add(row);
            }
            retVal.Add(data);
            ...
            if (HtmlDoc != null && HtmlDoc.HasData)
            {
                ...
                if (toSearch != null && toSearch.Any())
                {
                    foreach (ParseDataItem item in toSearch)
                    {
                        ...
                        {
                            Device newItem = new Device(Logger, DisplaySectionName, components);
                            if (newItem.HasData)
                                items.Add(newItem);
                            components = null;
                        }
        ...
        {
            internal Device(CommandLineLogger logger, string dataSectionName, IEnumerable<ParseDataItem> dataItems)
            {
                if (dataItems != null && dataItems.Any())
                {
                    foreach (ParseDataItem dataItem in dataItems)
                    {
                        ...
                        {
                            case "TYPE":
                                DeviceType = dataItem.Value;
                                break;
                            case "ID":
                                DeviceId = dataItem.Value;
                                break;
                            case "ACTIVE":
                                IsActive = dataItem.Value;
                                break;
                            default:
